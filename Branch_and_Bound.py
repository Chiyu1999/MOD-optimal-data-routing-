...
for n in range(10,15,5): 
    ...
    for times in range(1):
        ...
        def find_branch_and_its_upper_bound(i):
            ...
            #當需要分解的時候使用 -->　Bandwidth < 能提供的影片
            branch_1 = [[1,0,0],[0,1,0],[0,0,1]] #分支種類1
            branch_2 = [[1,1,0],[1,0,1],[0,1,1]] #分支種類2
            ...
            if(i == node_num-1):
                ...
                #判斷分支方法；列出可行分支 (bandwidth>= nums_video_provided)
        ...
        while(opt_flag==0):
            ...
            if(i==0):
                ...
                for j in range(len(all_count[i])):
                    if(all_count[i][j] == 0 ):                
                        all_count[i][j] = 1
                        if(j==0): change_count[i] =  all_count[i][0]
                        ...
                        continue #不需要繼續分支(預防temp_i增加)
                    else:
                        for v in range(all_count[i][j]):
                            if (all_upper_bound_list[i][temp_j]>temp_opt):                   
                                index_current[i] = temp_j #上游
                                temp_count[i] = temp_j
                                ...
                                for k in range(i-1,-1,-1):                                                                    
                                    if(index_current[k+1] >= change_count[k+1]): 
                                        ...
                                        temp_branch_upstream[k] = all_branch[k][index_current[k]]
                                all_count[i+1].append(0)
                                all_branch[i+1].append([0])
                                all_upper_bound_list[i+1].append(0)
                                all_temp_obj[i+1].append(0)
                                opt_count+=1
                            temp_j+=1     
                opt_flag=1
                ...
                for k in range(i+1):
                    change_count[k] = all_count[k][0]
                for k in range(len(all_count[i])):
                    temp_all_count.append(all_count[i][k])
                for j in range(len(all_count[i])):
                    if(all_count[i][j] == 0 ):                
                        all_count[i][j] = 1
                        if(j==0): change_count[i]=  all_count[i][0]
